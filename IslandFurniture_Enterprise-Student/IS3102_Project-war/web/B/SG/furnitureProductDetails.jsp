                    ...
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div>
                                    <div class="thumbnail">
                                        <img alt="" class="img-responsive img-rounded" src="../../..<%=furnitures.get(Id).getImageUrl()%>">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="summary entry-summary">
                                    <h2 class="shorter"><strong><%=furnitures.get(Id).getCategory()%></strong></h2>
                                    <%
                                        if (isMemberLoggedIn == true) {
                                    %>
                                    <form action="../../ECommerce_AddFurnitureToListServlet">
                                        <input type="hidden" name="id" value="<%=furnitures.get(Id).getId()%>"/>
                                        <input type="hidden" name="SKU" value="<%=furnitures.get(Id).getSKU()%>"/>
                                        <input type="hidden" name="price" value="<%=furnitures.get(Id).getPrice()%>"/>
                                        <input type="hidden" name="name" value="<%=furnitures.get(Id).getName()%>"/>
                                        <input type="hidden" name="imageURL" value="<%=furnitures.get(Id).getImageUrl()%>"/>
                                        <input type="submit" name="btnEdit" class="btn btn-primary" id="<%/*insert code here*/%>" value="Add To Cart"/>
                                    </form>
                                    <%}%>
                                    <p class="price"><h4 class="amount">$<%=furnitures.get(Id).getPrice()%></h4></p>
                                    <strong>Description</strong>
                                    <p class="taller">
                                        <%=furnitures.get(Id).getDescription()%>
                                    </p>
                                    <p>
                                        Height: <%=furnitures.get(Id).getHeight()%><br/>
                                        Length: <%=furnitures.get(Id).getLength()%><br/>
                                        Width: <%=furnitures.get(Id).getWidth()%>
                                    </p>
                                    <div class="product_meta">
                                        <span class="posted_in">Category: <a rel="tag" href="../../ECommerce_FurnitureCategoryServlet?cat=<%=furnitures.get(Id).getCategory()%>"><%=furnitures.get(Id).getCategory()%></a></span>
                                    </div>
                                    <br/><br/>
